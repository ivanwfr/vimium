/* ┌────────────────────────────────────────────────────────────────────────┐ */
/* │ background_scripts/main.js ...................... _TAG (260104:21h:33) │ */
/* └────────────────────────────────────────────────────────────────────────┘ */
...
/*{{{*/
/*{{{*/
const CSS_FUNCTION = "background-color: #000; border: 1px solid green; border-radius:1em; padding: 0 0.5em;";
const CSS_ERROR    =            "color: #FA0;                          border-radius:1em; padding: 0 0.5em; margin-left: 1em;";
const CSS_INFO     =            "color: #FF0;                          border-radius:1em; padding: 0 0.5em; margin-left: 1em;";
/*}}}*/
...
{
if(log_this) dom_log.console_clear("selectTab");
  chrome.tabs.query(visibleTabsQueryArgs, function(tabs) {
...
/*{{{*/
if(log_this) dom_log.log5("filtered_id=["+filtered_id+"] [url "+filtered_tabs[idx].url+"]");
...
if(log_this) {
  ...
  dom_log.log(direction+" "+tabs_idx+" [tabs.id "+tabs[tabs_idx].id+"] [url "+tabs[tabs_idx].url+"] "+(excluded ? "[excluded]":""));
}
/*}}}*/
...
/*}}}*/
        ...
        /*}}}*/
        /* current tab ● same_idx ● first next previous last {{{*/
/*{{{*/
if(log_this) dom_log.log_caller();
/*}}}*/
        let message;
        let   label = " "+(tabs_idx+1)+" / "+tabs.length;
        if( same_idx ) {
          message
            =   (direction == "next"    ) ? "🔴 \u25B6 last  tab"      +label
              : (direction == "previous") ? "🔴 \u25C0 first tab"      +label
              :                             "🔴 same "+direction+" tab"+label
        }
        else {
          message
            =   (direction == "next"    ) ? "🔴 \u25B6 "+direction+" tab"+label
              : (direction == "previous") ? "🔴 \u25C0 "+direction+" tab"+label
              :                             "🔴 "       +direction+" tab"+label
        }
        setTimeout(() => chrome.tabs.sendMessage(filtered_tabs[idx].id, { handler: "showMessage" , message }), 500);
if(log_this) dom_log.log(message);
        /*}}}*/
        return  tabs_idx;
      })();
      /*}}}*/
...
if(log_this) {
  ...
  console.log("%c swallowError( "+caller+" ) in background_scripts/main.js:", CSS_FUNCTION);
  console.log("%c"+message                                                  , CSS_ERROR   )
}
};
...
if(log_this) console.log("%c background_scripts/main.js ● chrome.runtime.onInstalled:", CSS_FUNCTION); /* eslint-disable-line newline-per-chained-call */
if(log_this) console.log("%c● "+ JSON.stringify(details).replace(/[{"}]/g," ").replace(/,/g,"\n●").trim(), CSS_INFO); /* eslint-disable-line newline-per-chained-call */
...
chrome.runtime.onSuspend.addListener(() => {
  if(log_this) console.log("chrome.runtime.onSuspend()");
});
// Background script
/*{{{
...
globalThis.background_main = background_main;
...
if(background_main.log_this) dom_log.log1("background_main.initializeExtension");
