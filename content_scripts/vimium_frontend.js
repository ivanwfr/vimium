/* ┌────────────────────────────────────────────────────────────────────────┐ */
/* │ vimium_frontend.js .............................. _TAG (251021:16h:03) │ */
/* └────────────────────────────────────────────────────────────────────────┘ */
...
const messageHandlers = {
  ...
  showMessage(request) {
if(log_this) dom_log.log4("vimium_frontend.sendMessage():\n"+ request.message);
...
/*{{{*/
  if( extensionHasBeenUnloaded() ) {
    //HUD.hide(true, false); // wont work (as beeing invalidated!)
if(log_this) dom_log.log_caller();
...
console.log("%c in content_scripts/vimium_frontend.js in checkIfEnabledForUrl()", "font-size: 200%; background-color: #808; border-radius: 1em; border: 1px solid white;");
    ...
    let delay_s = 2;
    let timeOut = setTimeout(() =>  document.location.reload(), delay_s * 1000);

    setTimeout(() => {
      if( confirm(  "🔴 Extension has been updated 🔴 RELOAD REQUIRED 🔴"
                  +  RELOAD_PROMPT)
        )
...
if(log_this) dom_log.log5("LOADED: content_scripts/vimium_frontend");

/*● name ● logging {{{*/
const  name = "vimium_frontend";
...
globalThis.vimium_frontend = vimium_frontend;

// ┌───────────────────────────┐
// │ DEBUG ● DevTools shortcut │
// └───────────────────────────┘
   globalThis.csvf  = globalThis.vimium_frontend;
if(globalThis.csvf.log_this) dom_log.log5("LOADED: csvf ● "+globalThis.csvf.name+" ● content_scripts/vimium_frontend.js");
